#!/usr/bin/env python3
"""Generate a K-12 science curriculum dictionary JSON from a PDF.

Usage:
  python scripts/generate_science_dictionary_k12_from_pdf.py \
    --pdf "C:/path/to/rika_list.pdf" \
    --output "lib/dictionaries/science_curriculum_k12.json"
"""
...
def ask_model_for_page(client: OpenAI, image_bytes: bytes, page_label: str) -> list[dict[str, Any]]:
    prompt = (
        "あなたは日本の理科カリキュラム抽出器です。"
        "画像内の単元一覧/系統表から、学年・領域・単元を具体語で抽出してください。"
        "JSON以外を返さないでください。"
        "\\n\\n出力形式:\\n"
        ...
        "- domain は短く（例: 物理, 化学, 生物, 地学, エネルギー, 生命, 地球）\\n"
        "- unit は具体的に（抽象語のみは不可）\\n"
